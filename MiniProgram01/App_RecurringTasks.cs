/*
 * 檔案功能：週期任務管理與自動推播模組 (SQLite 升級版 + 總覽/列印功能 - 標準展開排版)
 * 對應選單名稱：週期任務
 */
...
public class App_RecurringTasks : UserControl
{
    ...
    private void InitializeUI()
    {
        ...
        { 
            ...
            ForeColor = AppleGreen, 
            Font = new Font("Microsoft JhengHei UI", 10f, FontStyle.Bold), 
            Margin = new Padding(0, 5, 5, 5) 
        };
        btnViewAll.FlatAppearance.BorderSize = 1; 
        btnViewAll.FlatAppearance.BorderColor = AppleGreen;
    ...
    private void ExecutePrint()
    {
        ...
        { 
            ...
            Height = 600, 
            ShowIcon = false, 
            Text = "列印預覽 (可選擇 Microsoft Print to PDF 匯出)" 
        };
        ppd.ShowDialog();
    }
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            { 
                                ...
                                LastTriggeredDate = reader.GetString(5), 
    ...
    public async Task SaveTaskAsync(RecurringTask t)
    {
            ...
            await Task.Run(() => 
            {
                using (var conn = DatabaseManager.GetConnection()) 
                {
                    ...
                    using (var cmd = new SQLiteCommand(@"INSERT OR REPLACE INTO RecurringTasks (Id, Name, MonthStr, DateStr, TimeStr, LastTriggeredDate, Note, TaskType) VALUES (@Id, @Name, @MonthStr, @DateStr, @TimeStr, @LastTriggeredDate, @Note, @TaskType)", conn)) 
                    {
                        ...
                        cmd.Parameters.AddWithValue("@MonthStr", t.MonthStr); 
                        cmd.Parameters.AddWithValue("@DateStr", t.DateStr);
                        cmd.Parameters.AddWithValue("@TimeStr", t.TimeStr); 
                        cmd.Parameters.AddWithValue("@LastTriggeredDate", t.LastTriggeredDate);
        ...
        { 
            MessageBox.Show($"儲存任務失敗: {ex.Message}"); 
        }
    }
    ...
    public async Task DeleteTaskAsync(string id)
    {
        ...
        { 
            MessageBox.Show($"刪除任務失敗: {ex.Message}"); 
        }
    }
            ...
            await Task.Run(() => 
            {
                using (var conn = DatabaseManager.GetConnection()) 
                {
                    ...
                    using (var tx = conn.BeginTransaction()) 
                    {
                        ...
                        Set("DigestType", digestType); 
                        Set("DigestTimeStr", digestTimeStr); 
                        Set("LastDigestDate", lastDigestDate); 
                        Set("AdvanceDays", advanceDays.ToString()); 
                    ...
                    if (t.LastTriggeredDate != targetDateStr) 
                    {
                        ...
                        if (todoApp != null) 
                        {
                            if (parentForm.InvokeRequired) 
                                parentForm.Invoke(new Action(async () => await todoApp.ReceiveTaskAsync(prefix + t.Name, now.ToString("yyyy-MM-dd HH:mm:ss"), "Black", "")));
                            else 
                                await todoApp.ReceiveTaskAsync(prefix + t.Name, now.ToString("yyyy-MM-dd HH:mm:ss"), "Black", "");
                        }
                        t.LastTriggeredDate = targetDateStr; 
                        await SaveTaskAsync(t); 
                        needsRefresh = true;
...
public class AllTasksViewWindow : Form
{
    public AllTasksViewWindow(List<App_RecurringTasks.RecurringTask> tasks)
    {
        ...
        panel.Resize += (s, e) => 
        { 
            int safeWidth = panel.ClientSize.Width - 20; 
            if (safeWidth > 0) 
            { 
                foreach (Control c in panel.Controls) c.Width = safeWidth; 
            } 
        };
        ...
        foreach (var group in groupedTasks)
        {
            ...
            foreach (var t in group)
            {
                ...
                { 
                    Text = $"[{t.TaskType}] {t.Name} - {t.DateStr} {t.TimeStr}", 
        ...
        if (!tasks.Any()) 
            panel.Controls.Add(new Label() { Text = "目前沒有任何排程任務。", Font = new Font("Microsoft JhengHei UI", 10f), ForeColor = Color.Gray, AutoSize = true });
...
public class AddEditRecurringTaskWindow : Form
{
    ...
    public AddEditRecurringTaskWindow(App_RecurringTasks parent, App_RecurringTasks.RecurringTask item = null)
    {
        ...
        if (item != null && DateTime.TryParseExact(item.TimeStr, "HH:mm", null, System.Globalization.DateTimeStyles.None, out DateTime dtv)) 
            dtpTime.Value = dtv; 
        ...
        if (item != null && item.MonthStr != "特定日期" && cmbDate.Items.Contains(item.DateStr)) 
            cmbDate.SelectedItem = item.DateStr;
        if (item != null && item.MonthStr == "特定日期" && DateTime.TryParse(item.DateStr, out DateTime sDate)) 
            dtpSpecificDate.Value = sDate;
        ...
        btnSave.Click += async (s, e) => 
        {
            ...
            { 
                ...
                { 
                    ...
                    MonthStr = cmbCycleType.Text, 
                    DateStr = finalDateStr, 
                    TimeStr = dtpTime.Value.ToString("HH:mm"), 
                    LastTriggeredDate = "", 
                    Note = txtNote.Text, 
                    TaskType = cmbType.Text 
                }; 
            ...
            { 
                ...
                targetTask.Name = txtName.Text.Trim(); 
                targetTask.MonthStr = cmbCycleType.Text; 
                targetTask.DateStr = finalDateStr; 
                targetTask.TimeStr = dtpTime.Value.ToString("HH:mm"); 
                targetTask.Note = txtNote.Text; 
                targetTask.TaskType = cmbType.Text; 
            }
