...
namespace Safety_System
{
    public class App_WaterUsageDaily
    {
        ...
        public Control GetView()
        {
            ...
            GroupBox boxTop = new GroupBox { Text = $"【日】自來水使用量記錄 (庫：{DbName} 表：{TableName})", Dock = DockStyle.Fill, Font = new Font("Microsoft JhengHei UI", 12F), AutoSize = true, Padding = new Padding(10, 15, 10, 10) };
        ...
        private void UpdateCboColumns() { _cboColumns.Items.Clear(); foreach (DataGridViewColumn c in _dgv.Columns) if (c.Name != "Id" && c.Name != "日期") _cboColumns.Items.Add(c.Name); }
        private void EnforceMonthFormat(DataTable dt) { if (dt == null || !dt.Columns.Contains("月份")) return; foreach (DataRow row in dt.Rows) { if (row.RowState == DataRowState.Deleted) continue; if (DateTime.TryParse(row["月份"]?.ToString(), out DateTime d)) row["月份"] = d.ToString("yyyy-MM"); } }
        private void SetComboDate(ComboBox y, ComboBox m, ComboBox d, DateTime date) { if (y.Items.Contains(date.Year)) y.SelectedItem = date.Year; m.SelectedItem = date.Month.ToString("D2"); d.SelectedItem = date.Day.ToString("D2"); }
                ...
                if (sfd.ShowDialog() == DialogResult.OK) {
                    try {
                        ...
                        if (sfd.FilterIndex == 1) { using (ExcelPackage p = new ExcelPackage()) { var ws = p.Workbook.Worksheets.Add("Data"); ws.Cells["A1"].LoadFromDataTable(dt, true); p.SaveAs(new FileInfo(sfd.FileName)); } }
                            ...
                            StringBuilder sb = new StringBuilder(); sb.AppendLine(string.Join(",", dt.Columns.Cast<DataColumn>().Select(c => c.ColumnName))); foreach (DataRow r in dt.Rows) sb.AppendLine(string.Join(",", r.ItemArray.Select(i => i?.ToString().Replace(",", "，")))); File.WriteAllText(sfd.FileName, sb.ToString(), Encoding.UTF8);
                        }
            ...
            using (OpenFileDialog ofd = new OpenFileDialog { Filter = "CSV 檔案 (*.csv)|*.csv" }) {
                if (ofd.ShowDialog() == DialogResult.OK) {
                    try {
                        ...
                        DataTable dt = (DataTable)_dgv.DataSource; string[] headers = ParseCsvLine(lines[0]); _dgv.DataSource = null; _calcHelper?.BeginBulkUpdate();
                        foreach (string line in lines.Skip(1)) {
                            if (string.IsNullOrWhiteSpace(line)) continue;
                            DataRow nr = dt.NewRow(); string[] vs = ParseCsvLine(line);
                            for (int h = 0; h < headers.Length && h < vs.Length; h++) { string cn = headers[h].Trim(); if (dt.Columns.Contains(cn) && cn != "Id") nr[cn] = vs[h].Trim().Trim('"'); }
                            dt.Rows.Add(nr);
                        }
                        _calcHelper?.RecalculateTable(dt); _calcHelper?.EndBulkUpdate(); _dgv.DataSource = dt; RestoreColumnOrder();
        ...
        private void Dgv_KeyDown(object sender, KeyEventArgs e) {
            if (e.Control && e.KeyCode == Keys.V) {
                try {
                    string text = Clipboard.GetText(); if (string.IsNullOrEmpty(text)) return; _calcHelper?.BeginBulkUpdate();
                    ...
                    _calcHelper?.RecalculateTable(dt); _calcHelper?.EndBulkUpdate();
