...
namespace Safety_System
{
    public static class TableSchemaManager
    {
        ...
        public static readonly Dictionary<string, string> SchemaMap = new Dictionary<string, string>
        {
			...
			//水錶校正
            { "WaterMeterCalibration", "[日期] TEXT, [時間] TEXT, [水錶名稱] TEXT, [水錶位置] TEXT, [管徑] TEXT, [現場流量計讀值] TEXT, [檢驗單位讀值] TEXT, [器差值] TEXT, [設定參數K值] TEXT, [校正單位] TEXT, [下次校正日期] TEXT, [測定用途] TEXT, [附件檔案] TEXT, [備註] TEXT" },
			//其它檢測
