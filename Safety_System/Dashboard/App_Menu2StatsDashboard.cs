/// FILE: Safety_System/Dashboard/App_Menu2StatsDashboard.cs ///
using System;
...
{
    public class App_Menu2StatsDashboard
    {
        ...
        // 資料庫表名
        private const string TblThemes = "Menu2StatsDashboard_Themes";
        private const string TblConfigs = "Menu2StatsDashboard_Configs";
        private const string TblRecords = "Menu2StatsDashboard_Records";
        ...
        public App_Menu2StatsDashboard(string menuDbName)
        {
        ...
        public Control GetView()
        {
            ...
            Label lblTitle = new Label { Text = "📊 選單 2 - 動態統計看板", Font = new Font("Microsoft JhengHei UI", 24F, FontStyle.Bold), ForeColor = Color.DarkMagenta, Dock = DockStyle.Left, TextAlign = ContentAlignment.MiddleLeft, AutoSize = true };
        ...
        private void BuildThemeSection(int themeId, string themeName)
        {
            ...
            ui.Dgv.ColumnWidthChanged += (s, e) => {
                if (e.Column != null && e.Column.Width > 0) {
                    DataManager.SaveGridConfig("Menu2Stats", ui.ThemeName, "Width", e.Column.Name, e.Column.Width.ToString());
                    AdjustGridHeight(ui);
                }
            };
        ...
        {
            var widthDict = DataManager.LoadGridConfig("Menu2Stats", ui.ThemeName, "Width");
        ...
        private void Dgv_CellClick(object sender, DataGridViewCellEventArgs e, ThemeSectionUI ui)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0 && !ui.Dgv.Rows[e.RowIndex].IsNewRow) 
            {
                if (ui.Dgv.Columns[e.ColumnIndex].Name == "附件檔案") 
                {
                    ...
                    using (var frm = new AttachmentManagerUI(currentVal, "Menu2Stats", ui.ThemeId.ToString(), targetFolder, delegate(string path) { DeletePhysicalFile(path, ui.Dgv, e.RowIndex); })) {
