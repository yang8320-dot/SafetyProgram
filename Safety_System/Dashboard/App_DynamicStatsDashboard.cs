/// FILE: Safety_System/Dashboard/App_DynamicStatsDashboard.cs ///
using System;
...
{
    public class App_DynamicStatsDashboard
    {
        private string _menuDbName; 
        private string _prefix;
        private string _menuTitle;
        private Color _themeColor;
        private string _gridConfigKey;
        ...
        // 動態資料庫表名屬性
        private string TblThemes => $"{_prefix}StatsDashboard_Themes";
        private string TblConfigs => $"{_prefix}StatsDashboard_Configs";
        private string TblRecords => $"{_prefix}StatsDashboard_Records";
        ...
        public App_DynamicStatsDashboard(string menuDbName)
        {
            _menuDbName = menuDbName;
            _dbMap = App_DbConfig.GetDbMapCache();

            // 依據傳入的資料庫名稱，動態配置屬性與顏色 (完全相容舊版資料表)
            if (_menuDbName == "Menu1DB") { _prefix = "Menu1"; _menuTitle = "選單 1"; _themeColor = Color.DarkSlateBlue; _gridConfigKey = "Menu1Stats"; }
            else if (_menuDbName == "Menu2DB") { _prefix = "Menu2"; _menuTitle = "選單 2"; _themeColor = Color.DarkMagenta; _gridConfigKey = "Menu2Stats"; }
            else if (_menuDbName == "Menu3DB") { _prefix = "Menu3"; _menuTitle = "選單 3"; _themeColor = Color.SaddleBrown; _gridConfigKey = "Menu3Stats"; }
            else if (_menuDbName == "Menu4DB") { _prefix = "Menu4"; _menuTitle = "選單 4"; _themeColor = Color.DarkOliveGreen; _gridConfigKey = "Menu4Stats"; }
            else { _prefix = _menuDbName; _menuTitle = _menuDbName; _themeColor = Color.SteelBlue; _gridConfigKey = _menuDbName; }
        }
        ...
        public Control GetView()
        {
            ...
            Label lblTitle = new Label { Text = $"📊 {_menuTitle} - 動態統計看板", Font = new Font("Microsoft JhengHei UI", 24F, FontStyle.Bold), ForeColor = _themeColor, Dock = DockStyle.Left, TextAlign = ContentAlignment.MiddleLeft, AutoSize = true };
        ...
        {
            var widthDict = DataManager.LoadGridConfig(_gridConfigKey, ui.ThemeName, "Width");
        ...
        private void Dgv_CellClick(object sender, DataGridViewCellEventArgs e, ThemeSectionUI ui)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0 && !ui.Dgv.Rows[e.RowIndex].IsNewRow) 
            {
                if (ui.Dgv.Columns[e.ColumnIndex].Name == "附件檔案") 
                {
                    ...
                    using (var frm = new AttachmentManagerUI(currentVal, _gridConfigKey, ui.ThemeId.ToString(), targetFolder, delegate(string path) { DeletePhysicalFile(path, ui.Dgv, e.RowIndex); })) {
        ...
        private void OpenSettingsDialog(ThemeSectionUI ui)
        {
            using (Form f = new Form { Text = $"⚙️ 設定顯示與公式 - {ui.ThemeName}", Size = new Size(1380, 780), StartPosition = FormStartPosition.CenterParent, FormBorderStyle = FormBorderStyle.FixedDialog, MaximizeBox = false })
            {
                ...
                Label l2 = new Label { Text = "編輯選取項目的內容公式", Font = new Font("Microsoft JhengHei UI", 14F, FontStyle.Bold), ForeColor = _themeColor, Dock = DockStyle.Top, Height = 40 };
        ...
        private void BtnGlobalPdf_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog { Filter = "PDF 檔案 (*.pdf)|*.pdf", FileName = $"{_menuTitle}_動態看板報表_{DateTime.Now:yyyyMMdd}" })
            {
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    if (Form.ActiveForm != null) Form.ActiveForm.Cursor = Cursors.WaitCursor;
        ...
        private void BtnGlobalExcel_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog { Filter = "Excel 活頁簿 (*.xlsx)|*.xlsx", FileName = $"{_menuTitle}_動態看板報表_{DateTime.Now:yyyyMMdd}" })
            {
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try {
