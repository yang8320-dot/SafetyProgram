/// FILE: Safety_System/Dashboard/App_Menu1StatsDashboard.cs ///
using System;
...
{
    public class App_Menu1StatsDashboard
    {
        ...
        // 資料庫表名 (🟢 已統一更名為 Menu1 系列)
        private const string TblThemes = "Menu1StatsDashboard_Themes";
        private const string TblConfigs = "Menu1StatsDashboard_Configs";
        private const string TblRecords = "Menu1StatsDashboard_Records";
        ...
        public App_Menu1StatsDashboard(string menuDbName)
        {
        ...
        public Control GetView()
        {
            ...
            Label lblTitle = new Label { Text = "📊 選單 1 - 動態統計看板", Font = new Font("Microsoft JhengHei UI", 24F, FontStyle.Bold), ForeColor = Color.DarkSlateBlue, Dock = DockStyle.Left, TextAlign = ContentAlignment.MiddleLeft, AutoSize = true };
        ...
        private void BuildThemeSection(int themeId, string themeName)
        {
            ...
            ui.Dgv.ColumnWidthChanged += (s, e) => {
                if (e.Column != null && e.Column.Width > 0) {
                    DataManager.SaveGridConfig("Menu1Stats", ui.ThemeName, "Width", e.Column.Name, e.Column.Width.ToString());
                    AdjustGridHeight(ui);
                }
            };
        ...
        {
            var widthDict = DataManager.LoadGridConfig("Menu1Stats", ui.ThemeName, "Width");
        ...
        private void Dgv_CellClick(object sender, DataGridViewCellEventArgs e, ThemeSectionUI ui)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0 && !ui.Dgv.Rows[e.RowIndex].IsNewRow) 
            {
                if (ui.Dgv.Columns[e.ColumnIndex].Name == "附件檔案") 
                {
                    ...
                    using (var frm = new AttachmentManagerUI(currentVal, "Menu1Stats", ui.ThemeId.ToString(), targetFolder, delegate(string path) { DeletePhysicalFile(path, ui.Dgv, e.RowIndex); })) {
        ...
        private void BtnGlobalPdf_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog { Filter = "PDF 檔案 (*.pdf)|*.pdf", FileName = $"選單1_動態看板報表_{DateTime.Now:yyyyMMdd}" })
            {
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    if (Form.ActiveForm != null) Form.ActiveForm.Cursor = Cursors.WaitCursor;
        ...
        private void BtnGlobalExcel_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog { Filter = "Excel 活頁簿 (*.xlsx)|*.xlsx", FileName = $"選單1_動態看板報表_{DateTime.Now:yyyyMMdd}" })
            {
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try {
