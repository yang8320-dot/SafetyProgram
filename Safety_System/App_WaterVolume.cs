...
namespace Safety_System
{
    public class App_WaterVolume
    {
        ...
        public Control GetView()
        {
            ...
            // 🟢 更新標題
            GroupBox boxTop = new GroupBox { Text = $"自來水用水量統計 (庫：{DbName} 表：{TableName})", Dock = DockStyle.Fill, Font = new Font("Microsoft JhengHei UI", 12F), AutoSize = true, Padding = new Padding(10, 15, 10, 10) };
                ...
                if (int.TryParse(txtLatestCount.Text, out int limit) && limit > 0) {
                    ...
                    if (dt.Rows.Count > 0) {
                        ...
                        foreach(DataRow r in dt.Rows) {
                            // 🟢 日期解析改為 "月份"
                            if (DateTime.TryParse(r["月份"]?.ToString(), out DateTime d)) { 
                                if (d < minD) minD = d; 
                                if (d > maxD) maxD = d; 
                            }
                        }
        ...
        private void RefreshGrid() {
            if (_isFirstLoad) {
                ...
                if (dt.Rows.Count == 0) MessageBox.Show(Form.ActiveForm, "【系統連線成功】目前資料表尚無任何紀錄。", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    DateTime minD = DateTime.MaxValue, maxD = DateTime.MinValue;
                    // 🟢 日期解析改為 "月份"
                    foreach(DataRow r in dt.Rows) if (DateTime.TryParse(r["月份"]?.ToString(), out DateTime d)) { if (d < minD) minD = d; if (d > maxD) maxD = d; }
                    if (minD <= maxD) { _dtpStart.Value = minD; _dtpEnd.Value = maxD; }
                }
                _isFirstLoad = false;
            } else {
                // 🟢 過濾基準改為 "月份"
                _dgv.DataSource = DataManager.GetTableData(DbName, TableName, "月份", _dtpStart.Value.ToString("yyyy-MM-dd"), _dtpEnd.Value.ToString("yyyy-MM-dd"));
            }
            if (_dgv.Columns.Contains("Id")) _dgv.Columns["Id"].ReadOnly = true;
        ...
        private void BtnExport_Click(object sender, EventArgs e) {
            ...
            // 🟢 更新匯出檔案名稱前綴
            using (SaveFileDialog sfd = new SaveFileDialog { Filter = "Excel 活頁簿 (*.xlsx)|*.xlsx|CSV 檔案 (*.csv)|*.csv", FileName = "自來水用水量統計_" + DateTime.Now.ToString("yyyyMMdd") }) {
