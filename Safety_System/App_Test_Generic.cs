...
namespace Safety_System
{
    public class App_Test_Generic
    {
        ...
        // 🟢 導入自動運算 Helper (處理可能的日統計與星期推算，並加速大批次貼上)
        ...
        public Control GetView()
        {
            ...
            bDelCol.Click += (s, e) => { if (_cboColumns.SelectedItem != null && AuthManager.VerifyAdmin()) { DataManager.DropColumn(_dbName, _tableName, _cboColumns.SelectedItem.ToString()); RefreshGrid(); } };
        ...
        {
            if (dt == null || !dt.Columns.Contains("日期")) return;
            foreach (DataRow row in dt.Rows) {
                if (row.RowState == DataRowState.Deleted) continue;
                if (DateTime.TryParse(row["日期"]?.ToString(), out DateTime d)) 
                    row["日期"] = d.ToString("yyyy-MM-dd");
            }
        }
